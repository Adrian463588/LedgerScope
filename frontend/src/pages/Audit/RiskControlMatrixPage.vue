...
const { engagementApi, auditControlsApi } = useLedgerScopeApi();
...
const controls = ref<InternalControl[]>([]);
const selectedControl = ref<InternalControl | null>(null);
  ...
  try {
    ...
    loadError.value =
      caught instanceof Error
        ? caught.message
        : "Failed to load active engagement.";
...
async function fetchControls() {
  ...
  try {
    ...
    const firstControl = controls.value[0];
    if (firstControl && !selectedControl.value) {
      selectedControl.value = firstControl;
    }
  } catch {
...
async function handleCreateControl() {
  ...
  try {
    const newControl = await auditControlsApi.createControl(
      engagementId.value,
      {
        name: newName.value,
    ...
    notification.error(
      caught instanceof Error ? caught.message : "Failed to record control.",
    );
  }
}
...
async function updateEffectiveness(
  ...
  if (!engagementId.value) return;
  try {
    await auditControlsApi.updateControl(engagementId.value, control.id, {
      effectiveness: val,
    });
...
<template>
  <PageHeader
    ...
    subtitle="Map risks, internal controls, owners, and effectiveness testing."
  >
    <template #actions>
      <AppButton variant="primary" :icon="Plus" @click="showCreateModal = true"
        >New Control</AppButton
      >
    </template>
  </PageHeader>

  <div v-if="isLoading" class="p-6 text-gray-400">
    Loading Risk Control Matrix...
  </div>
  <div v-else-if="loadError" class="p-6 text-gray-400">
    {{ loadError }}
  </div>
  <div
  ...
  <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6 p-4">
    <div class="lg:col-span-2 space-y-4">
      <SectionPanel
        title="Controls List"
        subtitle="Configure and test engagement internal controls"
      >
              ...
              >
                ...
                <td class="py-3 px-2 text-right">
                  <button
                    ...
                    @click.stop="handleDeleteControl(ctrl.id)"
                  >
                    <Trash class="w-4 h-4" />
                  </button>
                </td>
    ...
    <div v-if="selectedControl && engagementId" class="space-y-4">
      <SectionPanel
        ...
        subtitle="Control definition & details"
      >
        <div class="text-sm text-gray-300 space-y-3">
          ...
          <div>
            ...
            >
            <p
              class="text-white bg-[color:var(--shell-950)] p-3 rounded border border-[color:var(--shell-border)] whitespace-pre-wrap"
            >
  ...
  >
    <div class="modal-content">
      <h3 class="text-lg text-white font-semibold mb-4">
        Record New Internal Control
      </h3>
      <div class="form-group">
        <AppInput
          v-model="newName"
          label="Control Name"
          placeholder="e.g. Segregation of duties for journal approval"
          required
        />
      </div>
      ...
      <div class="form-group">
        ...
        <textarea
          v-model="newDesc"
          class="form-textarea"
          placeholder="Describe control mechanism..."
          rows="3"
        ></textarea>
      </div>
      <div class="modal-buttons">
        <AppButton variant="secondary" @click="showCreateModal = false"
          >Cancel</AppButton
        >
        <AppButton variant="primary" @click="handleCreateControl"
          >Create Control</AppButton
        >
      </div>
    </div>
  </div>
