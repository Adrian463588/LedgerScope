...
import { Lock, Unlock, CheckSquare, MessageSquare, AlertCircle, FileText, Send, Plus, CornerDownRight } from 'lucide-vue-next';
...
const wpId = computed(() => Number(route.params['wpId']));
...
async function loadData(): Promise<void> {
  ...
  try {
    if (engagementId.value && wpId.value) {
      const data = await engagementApi.getWorkingPaper(engagementId.value, wpId.value);
      wp.value = data;
      editTitle.value = data.title;
      editContent.value = data.content || '';
      ui.setBreadcrumbs(['Audit', 'Working Papers', data.paper_ref || 'Detail']);
    } else {
      ...
      if (engagements.length > 0) {
        ...
        if (firstEng && firstEng.id !== undefined) {
          ...
          if (papers.length > 0 && papers[0] && papers[0].id !== undefined) {
            void vueRouter.push(`/engagements/${firstEng.id}/working-papers/${papers[0].id}`);
            return;
          }
        }
      }
...
async function handleSignOff(): Promise<void> {
  try {
    const data = await engagementApi.signOffWorkingPaper(engagementId.value, wpId.value);
    wp.value = data;
    notification.success('Working paper signed off successfully.');
  } catch (error: any) {
    notification.error(error.message || 'Failed to sign off working paper.');
  }
}
...
async function handleLock(): Promise<void> {
  const confirmed = await confirmDialog.confirm({
    ...
    message: 'Once locked, this working paper cannot be modified without documenting an unlock reason.',
    tone: 'danger',
  ...
  try {
    const data = await engagementApi.lockWorkingPaper(engagementId.value, wpId.value);
    wp.value = data;
    notification.success('Working paper locked.');
  } catch (error: any) {
    notification.error(error.message || 'Failed to lock working paper.');
  }
}
...
async function handleUnlock(): Promise<void> {
  ...
  try {
    const data = await engagementApi.unlockWorkingPaper(engagementId.value, wpId.value);
    wp.value = data;
    notification.success('Working paper unlocked.');
    showUnlockModal.value = false;
    unlockReason.value = '';
  } catch (error: any) {
    notification.error(error.message || 'Failed to unlock working paper.');
  }
}

async function saveChanges(): Promise<void> {
  try {
    const data = await engagementApi.updateWorkingPaper(engagementId.value, wpId.value, {
    ...
    notification.error(error.message || 'Failed to save changes.');
  }
}
...
async function addReviewNote(): Promise<void> {
  ...
  try {
    ...
    notification.error(error.message || 'Failed to add review note.');
  }
}
...
async function resolveNote(noteId: number): Promise<void> {
  try {
    ...
    notification.error(error.message || 'Failed to resolve review note.');
  }
}
...
async function sendReply(noteId: number): Promise<void> {
  ...
  try {
    ...
    notification.error(error.message || 'Failed to send reply.');
  }
}
...
<template>
      ...
      <div v-if="wpList.length > 0" class="wp-list">
        ...
        <ul>
          <li v-for="paper in wpList" :key="paper.id">
            <router-link :to="`/engagements/${engagementId}/working-papers/${paper.id}`">
  ...
  <div v-else>
    <PageHeader :title="`${wp.paper_ref || 'WP'} ${wp.title}`" subtitle="Working paper audit trail, sign-off controls, and review notes.">
      <template #actions>
        <StatusBadge :status="wp.status" />
        <AppButton v-if="wp.is_locked" variant="secondary" :icon="Unlock" @click="showUnlockModal = true">
          Unlock
        </AppButton>
        <AppButton v-else variant="secondary" :icon="Lock" @click="handleLock">
          Lock
        </AppButton>
        <AppButton v-if="!wp.is_locked && wp.status !== 'reviewed'" variant="primary" @click="handleSignOff">
    ...
    <div v-if="wp.is_locked" class="lock-banner">
      ...
      <span>This working paper is locked. Modifying content requires unlocking.</span>
    </div>
    ...
    <section class="wp-grid">
      <div class="main-column">
        <SectionPanel title="Working Paper content">
          <template #actions>
            <AppButton v-if="!isEditing && !wp.is_locked" variant="secondary" size="sm" @click="isEditing = true">
              Edit Content
            </AppButton>
            <div v-else-if="isEditing" class="edit-actions">
              <AppButton variant="secondary" size="sm" @click="isEditing = false">Cancel</AppButton>
              <AppButton variant="primary" size="sm" @click="saveChanges">Save</AppButton>
            </div>
          </template>

          <div v-if="!isEditing" class="wp-content-view">
            <h3>Audit Content</h3>
            <p class="content-text">{{ wp.content || 'No content written yet.' }}</p>
          </div>
          ...
          <div v-else class="wp-content-edit">
            ...
            <div class="textarea-wrapper">
              <label>Content</label>
              <textarea v-model="editContent" rows="10" class="strategy-textarea"></textarea>
            </div>
          </div>
        </SectionPanel>
        ...
        <SectionPanel title="Evidence Files" :icon="FileText">
          <div v-if="wp.evidence_files && wp.evidence_files.length > 0" class="evidence-list">
            <div v-for="file in wp.evidence_files" :key="file.id" class="evidence-item">
              <span class="file-name">{{ file.original_name }}</span>
              <span class="file-meta">({{ (file.file_size_bytes / 1024).toFixed(1) }} KB)</span>
              <StatusBadge :status="file.status" />
            </div>
          </div>
          <p v-else class="text-muted">No evidence files linked to this working paper.</p>
        </SectionPanel>
      </div>
      ...
      <div class="sidebar-column">
        <SectionPanel title="Sign-off Trail">
          <div class="trail-item">
            ...
            <span class="trail-value">{{ wp.prepared_by ? (wp.prepared_by.name || wp.prepared_by) : 'Not assigned' }}</span>
          </div>
          <div v-if="wp.reviewed_by" class="trail-item">
            <span class="trail-label">Reviewed By:</span>
            <span class="trail-value">{{ wp.reviewed_by.name || wp.reviewed_by }}</span>
          </div>
          <div v-if="wp.reviewed_at" class="trail-item">
            <span class="trail-label">Reviewed At:</span>
            <span class="trail-value">{{ new Date(wp.reviewed_at).toLocaleString() }}</span>
          </div>
        </SectionPanel>
        ...
        <SectionPanel title="Review Notes" :icon="MessageSquare">
          <div class="review-notes-list">
            <div v-for="note in wp.review_notes" :key="note.id" class="note-card">
              <div class="note-header">
                <span class="note-author">{{ note.created_by?.name || 'User' }}</span>
                <StatusBadge :status="note.status" />
              </div>
              <p class="note-text">{{ note.content }}</p>

              <!-- Nested replies -->
              <div v-if="note.replies && note.replies.length > 0" class="replies-list">
                <div v-for="reply in note.replies" :key="reply.id" class="reply-item">
                  <CornerDownRight class="reply-icon" />
                  <div class="reply-body">
                    <span class="reply-author">{{ reply.user?.name || 'User' }}:</span>
              ...
              <div class="reply-form">
                ...
                />
                <button @click="sendReply(note.id)"><Send class="send-icon" /></button>
              </div>

              <div v-if="note.status === 'open'" class="note-actions">
                <AppButton variant="secondary" size="sm" @click="resolveNote(note.id)">
                  Resolve Note
                </AppButton>
              </div>
            </div>
          ...
          <div class="add-note-form">
            ...
            <textarea v-model="newNoteContent" placeholder="New review note details..." rows="3"></textarea>
            <AppButton variant="secondary" size="sm" :icon="Plus" @click="addReviewNote">Add Note</AppButton>
          </div>
        </SectionPanel>
      </div>
    </section>
    ...
    <div v-if="showUnlockModal" class="modal-overlay">
      <div class="modal-content">
        ...
        <textarea v-model="unlockReason" rows="4" placeholder="Reason for unlocking..."></textarea>
        <div class="modal-buttons">
          <AppButton variant="secondary" @click="showUnlockModal = false">Cancel</AppButton>
          <AppButton variant="primary" @click="handleUnlock">Unlock Paper</AppButton>
        </div>
      </div>
    </div>
  </div>
