...
describe('design system components', () => {
  ...
  it('renders table skeleton and empty states', () => {
    ...
    expect(mount(AppTable, { props: { columns, data: [], loading: true } }).find('.skeleton').exists()).toBe(true);
    expect(mount(AppTable, { props: { columns, data: [], emptyText: 'Nothing here' } }).text()).toContain('Nothing here');
  });
});
