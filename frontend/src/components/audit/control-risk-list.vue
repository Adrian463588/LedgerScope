...
const { auditControlsApi } = useLedgerScopeApi();
import { useNotification } from "@/composables/useNotification";
import AppButton from "@/components/ui/AppButton.vue";
import AppInput from "@/components/ui/AppInput.vue";
import type { ControlRisk } from "@/types";
...
async function loadRisks() {
  ...
  try {
    risks.value = await auditControlsApi.listRisks(
      props.engagementId,
      props.controlId,
    );
  } catch {
...
async function handleAddRisk() {
  ...
  try {
    const newRisk = await auditControlsApi.addRisk(
      props.engagementId,
      props.controlId,
      {
    ...
    notification.error(
      caught instanceof Error ? caught.message : "Failed to link risk.",
    );
  }
}
...
async function handleDeleteRisk(riskId: number) {
  try {
    await auditControlsApi.deleteRisk(
      props.engagementId,
      props.controlId,
      riskId,
    );
    ...
    notification.error(
      caught instanceof Error ? caught.message : "Failed to remove risk.",
    );
  }
}
...
<template>
  <div class="control-risks-section">
    ...
    <div
      class="quick-add-risk bg-[color:var(--shell-900)] p-4 rounded-lg border border-[color:var(--shell-border)] mb-4"
    >
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
        <div class="md:col-span-2">
          <AppInput
            v-model="riskName"
            label="Risk Name"
            placeholder="e.g. Unauthorized journals bypass approval"
            required
          />
        </div>
        <div>
          <label class="block text-xs text-gray-400 font-medium mb-1"
            >Likelihood / Impact / Residual</label
          >
          <div class="flex gap-1">
            <select
              v-model="likelihood"
              class="bg-[color:var(--shell-950)] text-white text-xs rounded border border-[color:var(--shell-border)] p-1.5 flex-1"
            >
    ...
    <div v-else-if="risks.length === 0" class="text-gray-500 text-sm italic">
    ...
    <div v-else class="risks-list space-y-2">
      <div
        v-for="risk in risks"
        :key="risk.id"
        class="flex justify-between items-center p-3 rounded bg-[color:var(--shell-950)] border border-[color:var(--shell-border)]"
      >
        <div>
          ...
          <div class="flex gap-2 mt-1 text-xs">
            <span class="px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400"
              >Likelihood: {{ risk.likelihood }}</span
            >
            <span class="px-1.5 py-0.5 rounded bg-orange-500/20 text-orange-400"
              >Impact: {{ risk.impact }}</span
            >
            <span class="px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-400"
              >Residual: {{ risk.residual_risk || "medium" }}</span
            >
          </div>
        </div>
        <button
          class="p-1.5 text-red-400 hover:bg-red-500/10 rounded"
