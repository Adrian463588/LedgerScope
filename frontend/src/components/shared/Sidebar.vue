  ...
  >
    ...
    <div class="flex items-center justify-between min-h-[56px] px-[18px]">
      <span class="text-base tracking-normal text-[var(--brand-red)]">
        <strong class="text-white font-semibold">Ledger</strong
        ><i
          class="inline-block w-px h-[14px] mx-[6px] align-middle bg-[var(--brand-red)]"
        />Scope
      </span>
      <span
        v-if="!ui.sidebarCollapsed"
        class="text-[var(--text-inverse-muted)] font-mono text-[0.6875rem]"
        >v1.0</span
      >
      <button
        v-if="isMobileMenuOpen"
        class="md:hidden text-[var(--text-inverse-muted)]"
        @click="isMobileMenuOpen = false"
      >
        <X class="w-5 h-5" />
      </button>
    ...
    <nav class="flex-1 overflow-y-auto p-2">
      ...
      >
        <p
          v-if="!ui.sidebarCollapsed"
          class="m-0 text-[var(--text-inverse-muted)] text-[0.625rem] font-semibold tracking-[0.08em] opacity-50 px-3 pt-4 pb-1.5 uppercase"
        >
          {{ group }}
        </p>
        <NavLink
